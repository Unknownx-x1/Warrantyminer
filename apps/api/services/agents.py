...
def sanitize_untrusted_narrative(text: str) -> str:
    ...
    if not text:
        return ""
    # Strip potential delimiter injections and control characters
    cleaned = re.sub(r"[\r\n\t]+", " ", text)
...
class InvestigatorAgent(BaseAgent):
    ...
    def run(self, cluster_id: str, db: Session) -> Tuple[List[AgentFinding], List[ToolExecutionLog]]:
        ...
        # Tool Call 1: Load Cluster Claims
...
class RedTeamAgent(BaseAgent):
    ...
    def run(self, cluster_id: str, db: Session) -> Tuple[List[AgentFinding], List[ToolExecutionLog]]:
        ...
        # 1. Check Plant Concentration (Single-Plant Bias vs Fleetwide Issue)
        ...
        # 2. Check Counter-Examples & Cohesion
...
# -------------------------------------------------------------------------
# 5. CAPA & Deliverables Agent (8D & TSB Generator)
# -------------------------------------------------------------------------
class CAPAAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            ...
            role="capa",
            description="Compiles verified findings into formal 8D problem solving reports and Technical Service Bulletins."
        )
    ...
    def generate_8d_report(
        ...
        # Extract observed components, symptoms, plants, models from findings
        ...
        cond_finding = next((f for f in findings if f.agent_role == "investigator" and "condition" in f.statement.lower()), None)
        mech_finding = next((f for f in findings if f.agent_role == "investigator" and f.classification == "INFERRED"), None)
        trend_finding = next((f for f in findings if f.agent_role == "analytics" and "surge" in f.statement.lower()), None)
    ...
    def generate_tsb_draft(
        ...
        return {
            "tsb_id": f"TSB-REL-{datetime.utcnow().strftime('%Y')}-{cluster.cluster_index + 100:03d}",
            "cluster_id": cluster.id,
